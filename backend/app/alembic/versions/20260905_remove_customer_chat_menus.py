"""Remove customer-facing AI chat and conversation-history menus."""

import sqlalchemy as sa

from alembic import op

revision = "20260905_remove_customer_chat"
down_revision = "20260905_hide_model_config"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    bind.execute(sa.update(menu).where(menu.c.route_name == "AI").values(redirect="/ai/knowledge"))
    rows = bind.execute(sa.select(menu.c.id, menu.c.parent_id, menu.c.route_name, menu.c.component_path)).mappings().all()
    parent_by_id = {row["id"]: row["parent_id"] for row in rows}
    delete_ids = {
        row["id"]
        for row in rows
        if row["route_name"] in {"Chat", "Memory"}
        or row["component_path"] in {"module_ai/chat/index", "module_ai/memory/index"}
    }
