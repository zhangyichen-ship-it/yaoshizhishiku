# Yostone Knowledge · 前端工程（web）

基于 **Vue 3 + Vite + TypeScript + Element Plus** 的客户知识库管理前端，与 Yostone Knowledge FastAPI 后端配套使用。状态管理为 **Pinia**，样式以 **Tailwind CSS 4** 与 **SCSS** 为主，接口请求使用 **Axios**。
...
1. 先启动 **Yostone Knowledge 后端**，监听地址与 **`.env.development`** 里 **`VITE_API_BASE_URL`** 一致（模板默认为 **`http://127.0.0.1:8004`**）。
